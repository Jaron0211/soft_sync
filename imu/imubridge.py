...
class _383_unit():
	...
	def __init__(self,port):
		...
		self.ser = serial.Serial(port,115200,parity=serial.PARITY_NONE, rtscts=1)
		if self.ser.is_open:
			self.ser.close()
	...
	while(1):
		...
		if IMU1.trigger:
			...
			output = (
				...
				"q_X: %4f q_Y: %4f q_Z: %4f q_W: %4f"%(data[1][0],data[1][1],data[1][2],data[1][3]),
				"temp_X: %4f temp_Y: %4f temp_Z: %4f"%(data[2][0],data[2][1],data[2][2]),
				"board temp: %4f"%data[3],
				"timestamp: %d"%data[4],
				"Bitstate: %d"%data[5],
